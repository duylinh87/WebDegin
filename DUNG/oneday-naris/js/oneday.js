jQuery(function($) {
    ...
    $(function () {
        $('#oneday .item-action').click(function (event) {
            event.preventDefault();
            $("#oneday .oneday-content").removeClass("active");
            var url = $(this).attr('href');
            var dest = url.split('#');
            var target = dest[1];
            var target_offsets = $('#' + target).addClass("active");

            var target_offset = $('#' + target).offset();
            var target_top = target_offset.top;
            $('html, body').animate({scrollTop: target_top - 70}, 500, 'swing');
            return false;
        });
    });

    if($("#oneday").hasClass("template-oneday")){
        $("body").addClass("page-oneday");
    }
