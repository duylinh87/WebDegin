jQuery(function($) {
    ...
    var tabContent = $('.content-box'),
          tabContentActive = $('.content.active'),
          tabAnchorLinkItem = $('.anchor-link-list-item a');

    tabContentActive.addClass('open');
    tabAnchorLinkItem.on('click', function () {
        $(".animatedFadeInUp").removeClass("fadeInUp");
        var tabIndex = $(this).parent().index();
        tabAnchorLinkItem.removeClass('active');
        ...
        tabContent.eq(tabIndex).find(".mainImg-banner").addClass("fadeInUp");
    });
    ...
    $('.animatedFadeInUp').each(function () {
        ...
        var windowHeight = $(window).height();
        if (firstView > ptop - windowHeight + 100) {
            $(this).addClass('fadeInUp');
        }
    });

    if ($("body div").hasClass("memberPage")) {
        $("body").addClass("template-member");
    }
