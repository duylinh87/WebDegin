jQuery(function($) {
    ...
    $(window).scroll(function(){
        ...
        if(st > height50){
            $("#backTop").addClass("is-open");
        }else {
            $("#backTop").removeClass("is-open");
        }
    });

    $("#backTop .img_backTop").click(function () {
        $('html, body').animate({scrollTop: $('html, body').offset().top}, 'slow');
    });


    $( '#lp .link').click(function(){
        part = $(this).attr('href');
        console.log (part)
   ...
   $('#lp .item-number, #lp .merit, #lp .box-check ').addClass('js-fadein animated')
   $('#lp .item-event, #lp .box-voice, #lp .box-about, #lp .map .image_wrap').addClass('js-fadein animated')
   $('#lp .entry .heading, #lp .entry .title-01 ,#lp .entry .jeo-info , #lp .submbit').addClass('js-fadein animated')
